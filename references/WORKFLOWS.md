...
The manager handles an event as unacknowledged until `manager ack` is recorded. A successful paste or key return code is not Codex receipt; it only records a tmux delivery attempt. If the bound pane is unsafe, ambiguous, missing, dead, or no live Codex process is found, the notification stays `deferred`, `inject_pending`, or `inject_refused`; it never downgrades to `none`. The manager uses a short bound-pane tail capture as orchestration input: before injection deterministic guardrails decide whether the bound pane may receive the wake prompt, and after injection deterministic inspection may request a bounded submit-key follow-up only when the visible active composer text is the manager wake prompt. Each notification stores `wake_id` and `prompt_sha256`; `wake_id` is six lowercase hex characters in the first prompt line, followed by `;` so `ID:a0e140;tmux-skills...` remains parseable if the UI collapses a newline. If capture shows the same `ID:<wake_id>;` already visible in the Codex composer or queue, receipt recovery records `queued_in_codex` and does not reinject. If capture shows a different wake id, the manager resolves it through notification/event records: active unacknowledged or unknown wake ids block/defer for explicit TUI/CLI action, while wake ids whose events are acknowledged or handled are stale evidence and must not block the latest notification. Worker completion is determined from the status file and process/job state, not by reading large pane history. Main Codex must not directly monitor the worker pane or continue the repeat loop from the same turn after `manager run-next`; the manager is responsible for observing completion and sending the next wake event. A visible `Working` line does not prove receipt when the latest wake prompt block and composer footer still appear below it; with a `queue message` footer the bounded follow-up key is `Tab` only when that staged text is exactly the manager wake prompt. Main Codex should not infer a user queue from generic Codex composer text or placeholders; stale/handled checks come from manager state, not pane-screen guesses. Historical transcript prompts that start with `›` are not active composer text unless the block also shows an active composer footer or submit/queue hint before assistant output. Codex's dim placeholder suggestions, such as `Explain this codebase`, are not user drafts and are treated as an empty/safe composer when raw tmux capture preserves the placeholder style. Unrelated active composer text, partially typed user text, unknown staged content, or a different active/unknown wake id is an unsafe tmux-inject state: the manager must move the event to `deferred` or `blocked_by_other_wake`, record the reason, set `requires_manual_resume=true` when manual action is required, and avoid `C-m`/queue follow-up. A deferred event must not auto-resume just because the composer later becomes placeholder-only; use `manager notification retry|ack|discard|clear` or the TUI to resume or close it explicitly. This tmux-inject inspection path must not require `OPENAI_API_KEY` or a direct OpenAI API call. Clear terminal events and empty/safe bound panes use a narrow deterministic fast path. Ambiguous cases use a `uv` venv under the tmux-skills state dir, install `openai-codex`, and ask a small Python helper to call the SDK using defaults from `tmux-skills.config.json`; `TMUX_SKILLS_CODEX_SDK_MODEL` and `TMUX_SKILLS_CODEX_SDK_REASONING_EFFORT` override the file. The sidecar may assess pane safety, submit/queue state, and terminal-event evidence, but it must not choose a different pane, execute commands, or submit follow-up jobs by itself; main Codex still inspects and decides the next task after receiving the manager event. Deterministic guardrails override sidecar output for unsafe states: same wake id, different active/unknown wake id, unrelated composer text, user input, or unknown staged content must prevent retry/reinject even if sidecar says `retry`; stale acknowledged/handled wake ids are ignored for blocking. Sidecar setup or call failure falls back to deterministic wait/block for receipt recovery. If no ack arrives after a tmux delivery attempt, the manager keeps the event in `queued_in_codex`, `awaiting_receipt`, `receipt_blocked`, `blocked_by_other_wake`, or `deferred`, periodically rechecks the same event, and may retry the same wake prompt only when wake-id guardrails pass and the Codex sidecar judges retry appropriate from the bounded pane state. Each notification records timing data for sidecar calls, pane validation, paste/submit, capture, follow-up, receipt recheck, and total notify time. `submitted_event_ids` and `notified_event_ids` track delivery attempts, while `acknowledged_by_codex` is receipt and must stay consistent between the event summary and notification for the same event id.

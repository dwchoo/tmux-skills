...
The manager handles an event as unacknowledged until `manager ack` is recorded. A successful paste or key return code is not Codex receipt; it only records a tmux delivery attempt. If the bound pane is unsafe, ambiguous, missing, dead, or no live Codex process is found, the notification stays `inject_pending` or `inject_refused`; it never downgrades to `none`. The manager uses the bound tmux pane capture as orchestration input: before injection deterministic guardrails decide whether the bound pane may receive the wake prompt, and after injection deterministic inspection may request a bounded submit-key follow-up when the wake prompt remains staged in the composer. A visible `Working` line does not prove receipt when the latest wake prompt block and composer footer still appear below it; with a `queue message` footer the bounded follow-up key is `Tab` so the prompt is queued behind the active turn. This tmux-inject inspection path must not require `OPENAI_API_KEY` or a direct OpenAI API call. When `TMUX_SKILLS_CODEX_SIDECAR=1` is set, the manager creates or reuses a `uv` venv under the tmux-skills state dir, installs `openai-codex` there, and asks a small Python helper to call the SDK for structured decisions using the local Codex app-server/runtime and a read-only sandbox. The sidecar may assess pane safety, submit/queue state, and terminal-event evidence, but it must not choose a different pane, execute commands, or submit follow-up jobs by itself; main Codex still inspects and decides the next task after receiving the manager event. Deterministic guardrails override sidecar output: if the latest wake prompt block is still staged with a composer footer, the manager must not accept a sidecar `confirmed` decision and must run the bounded submit or queue follow-up before rechecking the pane. Sidecar setup or call failure falls back to deterministic inspection. If no ack arrives after a tmux delivery attempt, the manager keeps the event pending and periodically rechecks the same event instead of treating pane capture as receipt. `submitted_event_ids` and `notified_event_ids` track delivery attempts, while `acknowledged_by_codex` is receipt and must stay consistent between the event summary and notification for the same event id.

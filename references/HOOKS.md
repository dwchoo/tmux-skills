...
- `tmux-control bridge` is the app-server wake companion for dormant threads. It is not a hook replacement: it observes the same workspace state and sends only workspace/job/status/task/log paths to the operator-specified main Codex thread through the local app-server Unix socket WebSocket transport.

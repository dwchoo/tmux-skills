...
The tmux-inject prompt is intentionally not an instruction to run a command. It contains no shell commands, retry steps, long logs, output summaries, workspace paths, status paths, log paths, or task bodies. Injection uses tmux paste/send-keys only after deterministic guardrails verify the target pane still exists and is running Codex. Because a successful paste or key return code does not prove that Codex received a turn, the manager records `submitted_to_tmux` for pane injection attempts and waits for `manager ack` as the only receipt signal. The manager then inspects the bound pane capture itself and may perform a bounded submit-key follow-up when the wake prompt is still visible in the composer. A visible `Working` line does not prove receipt if the latest wake prompt block and composer footer still appear below it; with a `queue message` footer the follow-up key is `Tab` so the prompt is queued behind the active turn. This tmux-inject inspection path must not require `OPENAI_API_KEY` or a direct OpenAI API call. When `TMUX_SKILLS_CODEX_SIDECAR=1` is set, the manager creates or reuses a `uv` venv under the tmux-skills state dir, installs `openai-codex` there, and asks a small Python helper to call the SDK for structured decisions using the local Codex app-server/runtime and a read-only sandbox. The sidecar may assess pane safety, submit/queue state, and terminal-event evidence, but it must not choose a different pane, execute commands, or submit follow-up jobs by itself; main Codex still inspects and decides the next task after receiving the manager event. Deterministic guardrails still own pane selection, safety, and staged-composer detection: if the latest wake prompt block is still staged with a composer footer, sidecar `confirmed` is ignored and the bounded submit or queue follow-up runs before the pane is rechecked. Sidecar setup or call failure falls back to deterministic inspection. Missing `manager ack` keeps the notification `inject_pending` instead of falling back to diagnostics-only mode or treating pane capture as receipt.

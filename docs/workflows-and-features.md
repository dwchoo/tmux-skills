...
| MCP manager | Submit and observe manager-owned work through opaque handles and bounded grants so Codex cannot discover worker panes or raw evidence unless the user explicitly asks. | `manager.start`, `manager.submit`, `manager.status`, `manager.observe`, `manager.ack`, `manager.run_next`, `manager.cancel` | This document |
...
- The MCP tools are the official Codex-facing path for manager-owned work: `manager.start`, `manager.submit`, `manager.status`, `manager.observe`, `manager.ack`, `manager.run_next`, and `manager.cancel`. The legacy `tmux_control.py manager ...` CLI remains for compatibility, demos, and manual diagnostics, but it must enforce the same read boundary for manager-owned jobs.
- `manager.submit` returns an opaque `job_handle` and a `codex_contract`. It must not return worker `pane_id`, `status_path`, `log_path`, command-file path, or raw terminal output. The default contract is `observe: none`.
- `manager.status` reports bounded manager/job/event state. For manager-owned jobs it returns redacted evidence references unless a valid observe grant is supplied.
- `manager.observe` is the only normal way for Codex to read manager-owned status/log/output details. It may issue a one-shot read grant when the user explicitly asks to see progress or logs. Continuous monitoring is allowed only when the user explicitly requests it, and every lease must include finite `ttl_seconds`, `interval_seconds`, and `max_reads`.
- Terminal event wakeups may carry only an event-scoped one-time read token. That token can reveal the details needed to handle that event and must not be reusable after `manager.ack` or after its first successful one-shot read. Manager events that carry no read token, such as bridge receipt preflight, require only bounded `manager.status` and `manager.ack`; Codex must not invent an observe token for them.
- `manager.ack` records receipt for one event, consumes event-scoped read grants, and must be reflected in both the event and notification state. `manager.run_next` records the prior event as handled only after required ack/read constraints pass.
- Every manager-owned read, denied read, observe grant, grant reuse denial, manual override, `ack`, `run_next`, and cancellation decision is appended to an audit log under the tmux-skills state directory.
- Manager-owned job metadata is durable in both manager records and job/status records. CLI guards and hooks must still be able to recognize manager ownership after manager cleanup removes the manager record.

Plugin distribution contract:

- A plugin distribution may bundle this `SKILL.md`, the stdlib MCP manager server entrypoint, MCP server config, and lifecycle hooks together so a workspace can install one tmux-skills unit.
- The MCP server must run from the local Python scripts in this repository or installed skill directory. Docker-based MCP servers or Docker-based installation steps are not supported.
- The repo-local stdio MCP entrypoint is `python scripts/tmux_mcp_server.py`. It exposes the seven manager tools through MCP `tools/list` and `tools/call`.
- The project-scoped MCP server name is `tmux-manager`. Avoid `tmux-skills` as the MCP server name because it can be confused with Skills, and do not install the legacy global `tmux-control` Skill for manager workflows.
- For project-only use, place the MCP server table in the repository's `.codex/config.toml`; do not use `codex mcp add`, because that writes user-level MCP config.
- The repo-local hook entrypoint is `python scripts/codex_tmux_hook.py`; use `context --event SessionStart`, `context --event UserPromptSubmit`, `pre-tool-use`, and `stop` for the four lifecycle policies.
- Repo-local hooks and plugin-bundled hooks follow the same policy: `PreToolUse` blocks arbitrary manager-owned polling/reads without a grant, `UserPromptSubmit` and `SessionStart` inject the no-polling context, and `Stop` defers manager-owned terminal fallback handling to the manager event lifecycle.
- The skill remains a thin instruction layer. Durable enforcement belongs in the MCP manager tools, compatibility CLI guards, hooks, and audit log.
...
- Each internal job record stores its target `pane_id`, best-known `pane_index`, status path, log path, command request path, start metadata, terminal event metadata, and durable manager-owned metadata. These raw fields are manager-internal evidence, not Codex-facing submit output. Manager-owned logs are bounded by `log_max_bytes` and are trimmed to a tail while the manager is alive, so log files do not grow without limit. When Codex receives a manager wake event, it should use the MCP manager handle and event token, inspect manager state once, acknowledge or handle only that event, and avoid direct `capture`, raw `cat`, or repeated `manager status` polling unless the user explicitly requested diagnostics and a matching observe grant exists.
...
The dashboard is a compact tmux TUI rendered by a single stdlib curses viewer process. The job `pane` column shows the best-known tmux pane index with the stable pane id as `index:%id`, for example `2:%11`; if the index is unknown it falls back to `%id`. 기본 `summary` 화면은 6-8줄 안팎의 요약만 표시하고, full path, command file path, status/log path, bridge delivery ids, 오래된 event/job 목록, diagnosis, log body, traceback은 표시하지 않는다. `Tab`은 `summary`, `jobs`, `events` mode를 순환하고, `1`, `2`, `3`은 각 mode로 직접 이동한다. Textual/Rich TUI를 사용할 수 있으면 pending/deferred/acknowledged notification 목록과 선택 항목 상세 reason/timing을 보여주고, 명시 action으로 `retry`, `ack`, `discard`, `clear acknowledged/completed`를 실행할 수 있다. `d`, `D`, 또는 Delete는 terminal job 중 `succeeded`, `complete`, `completed`, `failed`, `error`, `stopped`, `cancelled`, `timeout`, `stale` 상태만 dashboard에서 삭제하고 manager-owned command/status/log evidence만 제거한다. Active job, panes, windows, manager process는 건드리지 않는다. `q`는 viewer만 종료하려고 시도하며 manager process나 worker job은 건드리지 않는다. 상세 원문 증거가 필요하면 사용자의 명시 요청에 따라 `manager.observe` 또는 compatibility `manager observe` grant를 먼저 발급한다.
...
- With bridge notification enabled, the manager submits a bounded wake prompt for each terminal event, then waits for event acknowledgement. For manager-owned jobs the prompt includes only manager/job/event handles and, when needed, an event-scoped one-time read token. It must not include worker pane IDs, status/log paths, summaries, task instruction bodies, diagnosis, retry commands, or model/delegation instructions. Codex acknowledges receipt by running `manager.ack` or compatibility `manager ack` for that event immediately after the allowed event-scoped read is complete, before writing reports or taking follow-up actions; if the prompt has no read token, Codex acknowledges after one bounded status inspection.
...
The tmux-inject prompt is intentionally not an instruction to run a shell command. It contains no shell commands, retry steps, long logs, output summaries, workspace paths, status paths, log paths, pane IDs, or task bodies. It tells Codex to handle only the latest unacknowledged event and to ack/report only when the event is stale or already handled. In manager-controlled mode, Codex must inspect manager status once after this prompt, then stop after ack/report or one `manager.run_next`; polling the worker pane or repeatedly polling `manager.status` is a demo failure unless the user explicitly asks for diagnostics and `manager.observe` grants a bounded read. Injection uses tmux paste/send-keys only after deterministic guardrails verify the target pane still exists and is running Codex and the bound pane capture does not show active user/composer text that could be submitted accidentally. Because a successful paste or key return code does not prove that Codex received a turn, the manager records `submitted_to_tmux` for pane injection attempts and waits for `manager.ack` as the only receipt signal. Each wake prompt has a stable six-hex `wake_id`, a trailing `;` delimiter, and `prompt_sha256`; when bounded capture shows the same wake id already present in the active composer or Codex queue, receipt recovery records `queued_in_codex` and must not reinject. When bounded capture shows a different wake id, the manager resolves it through the notification/event record: active unacknowledged or unknown wake ids become `blocked_by_other_wake` or `deferred`, while wake ids that map only to acknowledged or handled events are stale evidence and do not block the latest notification. The manager then inspects a short bound-pane tail capture and may perform a bounded submit-key follow-up only when the visible active composer content is the manager's own wake prompt; if active user text, an unrelated active prompt, or an ambiguous active composer state is visible, the event becomes `deferred` with `requires_manual_resume=true` and no `C-m` or queue key is sent. A deferred event is not automatically resumed later when the composer returns to an empty placeholder; it requires `manager notification retry`, `manager notification ack`, `manager notification discard`, or an equivalent TUI action. Worker completion is determined from status files and job/process state, not by reading large pane history. Main Codex must not directly monitor the worker pane or continue the repeat loop from the same turn after `manager.run_next`; the manager is responsible for observing completion and sending the next wake event. A visible `Working` line does not prove receipt if the latest wake prompt block and composer footer still appear below it; composer footer detection includes submit/queue/context hints and the latest Codex model/status line such as `gpt-... · /path`. With a `queue message` footer the follow-up key is `Tab` only when the staged text is exactly the manager wake prompt. Generic Codex composer text or placeholders are not a stop condition for the repeat loop, but non-empty active user/composer text is a tmux-inject safety risk; stale/handled checks come from manager state. Historical Codex transcript prompts that start with `›` are not active composer text by themselves and must not block injection unless the bounded capture also shows an active composer footer/submit hint before any assistant output marker. Codex's placeholder suggestions, such as dim `Explain this codebase` or unstyled defaults like `Summarize recent commits` and `Run /review on my current changes`, are not user drafts and are treated as an empty/safe composer when the bounded capture identifies them as placeholder UI. The sidecar must follow that structured `placeholder_composer_suggestion` status and must not reinterpret the placeholder preview as user text; deterministic composer-state safety overrides a sidecar block based only on placeholder wording. Receipt sidecar input uses only the recent Codex UI tail, capped to about 15 lines and 1200 characters after placeholder sanitization, because receipt recovery only needs current working/queued/composer state. This tmux-inject inspection path must not require `OPENAI_API_KEY` or a direct OpenAI API call. Clear terminal events and empty/safe bound panes use a narrow deterministic fast path. Ambiguous cases create or reuse a `uv` venv under the tmux-skills state dir, install `openai-codex`, and ask a small Python helper to call the SDK using defaults from `tmux-skills.config.json`; `TMUX_SKILLS_CODEX_SDK_MODEL` and `TMUX_SKILLS_CODEX_SDK_REASONING_EFFORT` override the file. The sidecar may assess pane safety, submit/queue state, terminal-event evidence, and receipt recovery, but it must not choose a different pane, execute commands, or submit follow-up jobs by itself; main Codex still inspects and decides the next task after receiving the manager event. Deterministic guardrails still own pane selection, safety, staged-composer detection, and wake-id dedupe: same wake id means do not reinject, active unacknowledged or unknown different wake id means block/defer, stale acknowledged or handled wake id is ignored for blocking, and sidecar `retry` is accepted only after those guardrails pass. Missing `manager.ack` keeps the notification in `queued_in_codex`, `awaiting_receipt`, `receipt_blocked`, `blocked_by_other_wake`, or `deferred`; receipt recovery runs at the configured interval only for non-deferred notifications, asks the Codex sidecar after deterministic wake-id checks, retries only on explicit sidecar `retry`, stops automatic checks on `block` or check/retry limits, and waits on sidecar failure or uncertain/working states. Each notification stores timing data for sidecar setup/calls, pane validation, paste/submit, capture, follow-up, receipt recheck, and total notify time. Debug sidecar trace, controlled by `tmux-skills.config.json` or `TMUX_SKILLS_DEBUG_SIDECAR`, additionally preserves per-check sidecar history and bounded raw decision data so repeated `wait` or unavailable decisions can be diagnosed without increasing normal-mode records.
- Current Codex placeholder examples also include `Improve documentation in @filename`, `Find and fix a bug in @filename`, `Implement {feature}`, and `Use /skills to list available skills`; they are treated as placeholder UI, not active user text.
- Normal operation must not rely on Codex polling `manager.status`. Main Codex should learn about terminal work from a manager event, then use only `manager.status`, `manager.observe`, `manager.ack`, and `manager.run_next` according to the event contract.
...
- Codex lifecycle hooks are optional fallback guards, not the default manager wake path. `PreToolUse` must block arbitrary monitoring commands while active delegated manager jobs exist, including raw `sleep`-then-status loops, repeated `manager status`, direct `.codex/tmux-skills` reads, raw status/log `cat`, and `tmux capture` against manager-owned panes. It may allow a command only when it carries a valid observe grant or an explicit manual override with a reason. `UserPromptSubmit` and `SessionStart` inject additional context that forbids arbitrary polling and allows only one manager-mediated observe when the user explicitly asks for progress/logs. `Stop` must not block manager-owned raw terminal statuses as a duplicate fallback; the manager event lifecycle owns notification, retries, ack, and run-next. Seeing `<hook_prompt ...>tmux-skills observed a terminal event...` for a manager-owned event is a duplicate fallback notification, not a new manager request.
...
python scripts/tmux_control.py manager observe --job-handle JOB_HANDLE --once --reason 'user requested progress'
python scripts/tmux_control.py manager bridge-check
...
`tmux_control.py bridge`는 `.codex/tmux-skills/status`의 terminal event와 `.codex/tmux-skills/tasks`의 ready task를 관찰하고, 같은 local `codex app-server`에 연결된 사용자가 지정한 main Codex thread에 wake prompt만 제출한다. Bridge는 visible manager pane과 명시적 `task load --for-skill`로 부족한 dormant wakeup만 담당한다.
...
```text
tmux-manager observed a terminal event.
...
Ready task prompts use `tmux-manager observed a ready task.` as the first line. The prompt must not include status/log summaries, `last_output`, stdout/stderr tails, traceback text, task instruction bodies, diagnosis, suggested commands, retry commands, or model/delegation language.
...
| MCP manager submit hides pane/log/status evidence and returns opaque handles | `tests/test_tmux_manager.py` MCP contract tests |
| Manager observe grants one-shot or bounded reads and rejects reuse | `tests/test_tmux_manager.py` MCP contract tests |
| CLI and hooks block manager-owned raw reads without grant or manual override | `tests/test_tmux_control.py`, `tests/test_codex_tmux_hook.py` |

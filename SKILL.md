...
- `manager start --notify tmux-inject` binds to exactly one Codex pane with `--codex-pane PANE_ID`; `--codex-pane current` is allowed only when the current tmux pane resolves to a live Codex process. On a terminal event, the manager pastes only a short wake prompt containing the manager id, event id, `$tmux-control` instruction, the rule to handle only the latest unacknowledged event, and the rule to wait for the next manager event after `manager run-next`. If the event is stale or already acknowledged/handled, Codex should ack/report only and must not run the next job. Codex should not treat generic composer text, placeholders, or historical transcript prompts as a stop condition. Dim Codex suggestion placeholders such as `Explain this codebase` are safe empty composer content when raw tmux capture preserves the placeholder style. The manager may submit or follow up with a submit key only when pane capture shows no unrelated active user/composer text or shows exactly the manager's wake prompt staged in the active composer; ambiguous or user-entered active text keeps the event pending and no key is sent. The prompt contains no shell commands, retry instructions, long logs, output summaries, path dumps, or task bodies. Until `manager ack` is recorded, the event remains unacknowledged in `awaiting_receipt`; after the first delivery attempt, empty/placeholder composer state alone is not enough to retry, and the manager must ask the Codex sidecar to judge whether retry, wait, or block is appropriate.

...
8. For the default visible long-task workflow, start one idle Codex-owned manager first with `python scripts/tmux_control.py manager start [--manager-id <id>] --notify bridge --thread-id THREAD --endpoint unix://PATH`, submit each long command with `python scripts/tmux_control.py manager run-next --job-id <id> --command '<command>'`, and acknowledge bridge turns with `python scripts/tmux_control.py manager ack --event-id EVENT`.
...
- The current supported long-task workflow is `foreground-debug`: one Codex-owned foreground manager process plus one long worker pane and one compact manager pane. Keep `manager start` running only for debug/demo use; it keeps Codex working while it runs. The intended `background-operating` mode is gated on `manager ps-poc` proof that Codex can launch a manager as a Codex-owned background terminal visible in `/ps` while the main Codex thread is idle.
- Start the manager before starting long work. The manager may start idle, then Codex verifies bridge receipt with `manager bridge-check`, then submits the first and later scripts with `manager run-next`; the manager tracks each job through the same worker pane.
...
- If the same manager process is already alive, a repeated `manager start` queues work to that process and exits instead of starting a second manager loop. `manager start --process-mode background` must fail with `unsupported_by_current_codex_surface` until `manager ps-poc` proves the launch surface; do not silently daemonize outside Codex because that would not satisfy `/ps` visibility or Codex-owned shutdown.
- The manager process starts worker jobs through `tmux_control.py run` and updates dashboard text without leaving a persistent tmux renderer process. Worker jobs run in the tmux worker pane and continue if the Codex-owned manager exits. The manager pane is a display surface, not proof that the manager process is Codex-owned.
- Manager-owned job logs are bounded by `log_max_bytes` so the log file does not grow without limit. The manager dashboard and status files keep paths small; when Codex receives a bridge turn, it should first read the manager path from the prompt, acknowledge the current target event with `manager ack`, then use `capture` on the worker pane for live terminal output and only read bounded status/log files as needed. For bridge preflight prompts, acknowledge `bridge_verification.event_id`; for terminal prompts, acknowledge `last_terminal_event_id`.
- When a manager observes `succeeded`, `failed`, `stopped`, `timeout`, `cancelled`, `stale`, or a missing worker pane, it records `waiting_for_codex`, submits a path-only bridge notification for that terminal event, and waits for main Codex to inspect paths and decide the next action. Normal operation must not rely on Codex polling `manager status`; Codex should learn about terminal work from the manager's bridge turn.
...
python scripts/tmux_control.py manager start [--manager-id ID] [--job-id ID (--command TEXT|--command-file PATH)] [--notify bridge --thread-id THREAD --endpoint unix://PATH|--notify none] [--log-max-bytes N] [--process-mode foreground|background]

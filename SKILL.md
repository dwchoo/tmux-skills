...
- Manager-owned job logs are bounded by `log_max_bytes` so the log file does not grow without limit. The manager dashboard and status files keep paths small; when Codex receives a bridge turn, it should first acknowledge the event with `manager ack`, then use `capture` on the worker pane for live terminal output and only read bounded status/log files as needed.

...
- The manager process starts worker jobs through `tmux_control.py run`, writes atomic dashboard snapshots, and starts or reuses one viewer in the manager pane. Dashboard job rows show a readable pane label as `index:%id` when the tmux pane index is known, falling back to `%id`. Prefer the `uv`-managed Textual/Rich TUI when available, and keep the stdlib compact viewer as the fallback. Do not refresh the manager pane by repeatedly injecting `clear; cat` or similar `tmux send-keys` commands. The only allowed pane text injection is the explicit tmux-inject wake backend into the verified Codex pane. Worker jobs run in the tmux worker pane and continue if the Codex-owned manager exits. The manager pane is a compact display surface, not proof that the manager process is Codex-owned.
...
- `manager start --notify tmux-inject` binds to exactly one Codex pane with `--codex-pane PANE_ID`; `--codex-pane current` is allowed only when the current tmux pane resolves to a live Codex process. This is manager-controlled mode: Codex handles manager work only after a manager event prompt, inspects manager state once, acknowledges/reports the event, optionally queues one `manager run-next`, then stops and waits for the next manager event. Direct polling/monitoring is reserved for explicit user diagnostics. On a terminal event, the manager pastes only a short wake prompt whose first line is `ID:<wake_id>;` where `wake_id` is six lowercase hex characters, and whose body contains the manager id, event id, `$tmux-control` instruction, the rule to handle only the latest unacknowledged event, and the rule to wait for the next manager event after `manager run-next`. If the event is stale or already acknowledged/handled, Codex should ack/report only and must not run the next job. Codex should not treat generic composer text, placeholders, or historical transcript prompts as a stop condition. Dim Codex suggestion placeholders such as `Explain this codebase` are safe empty composer content when raw tmux capture preserves the placeholder style. The manager may submit or follow up with a submit key only when pane capture shows no unrelated active user/composer text, no different active unacknowledged queued wake id, or shows exactly the same manager wake prompt staged in the active composer. If the same `wake_id` is already visible, the manager records `queued_in_codex` and must not reinject. A different visible wake id that maps to an unacknowledged notification, an unknown visible wake id, ambiguous composer, or user-entered active text moves the event to `blocked_by_other_wake` or `deferred` with `requires_manual_resume=true`; a visible wake id that maps only to an acknowledged or handled event is stale evidence and must not block the latest event. The prompt contains no shell commands, retry instructions, long logs, output summaries, path dumps, or task bodies. Until `manager ack` is recorded, the event remains unacknowledged in `queued_in_codex`, `awaiting_receipt`, `receipt_blocked`, `blocked_by_other_wake`, or `deferred`; after the first delivery attempt, empty/placeholder composer state alone is not enough to retry, and the manager must classify visible wake ids before asking the Codex sidecar to judge ambiguous retry/wait/block cases.
- Receipt recovery defaults live in `tmux-skills.config.json`: recheck every 10 seconds, ask the sidecar at most 6 times, and retry injection at most once. `receipt_blocked` stops automatic rechecks until the user explicitly acks, cancels, or restarts. Receipt sidecar input is intentionally short, using only the recent Codex UI tail, capped to about 15 lines and 1200 characters after placeholder sanitization. When debug sidecar trace is enabled, the manager preserves per-check receipt sidecar history with action, reason, source, confidence, timing, and bounded raw decision data; normal mode keeps only compact current-state fields. The sidecar must treat `composer_state.status=placeholder_composer_suggestion` as idle/safe placeholder UI, not user text or unrelated composer content; deterministic composer-state safety overrides a sidecar block that is based only on placeholder text.
- The tmux-inject manager uses deterministic guardrails and bound-pane capture inspection for wake orchestration. It may inject only into the bound Codex pane, and after injection it may perform a bounded submit-key follow-up only when the wake prompt was pasted but not submitted and no unrelated composer text is visible. This path must not require `OPENAI_API_KEY` or direct OpenAI API access. Clear terminal events and empty/safe bound panes use a narrow deterministic fast path without an SDK call; ambiguous cases use a `uv` venv under the tmux-skills state dir, install `openai-codex`, and ask a small Python helper to call the SDK for structured decisions. The sidecar defaults live in `tmux-skills.config.json` and currently use model `gpt-5.5`, low reasoning, and deterministic fast path enabled; `TMUX_SKILLS_CODEX_SDK_MODEL` and `TMUX_SKILLS_CODEX_SDK_REASONING_EFFORT` override the file. The sidecar may assess pane safety, submit/queue state, and terminal-event evidence, but it must not choose a different pane, execute commands, or submit follow-up jobs by itself; main Codex still inspects and decides the next task after receiving the manager event. Deterministic guardrails override sidecar output for unsafe states: same `wake_id` already visible, another active unacknowledged or unknown `wake_id` already visible, unrelated composer text, user input, or an unknown staged prompt must prevent reinjection even if sidecar says `retry`; acknowledged or handled visible wake ids are stale evidence and do not block newer notifications. Sidecar setup or call failure falls back to deterministic defer for unsafe composer states. Missing `manager ack` keeps the event `queued_in_codex`, `awaiting_receipt`, `receipt_blocked`, `blocked_by_other_wake`, `deferred`, `inject_pending`, or `inject_refused`; the manager must not fall back to `--notify none` or treat pane capture as receipt. Use `manager notification list|retry|ack|discard|clear` or the manager TUI to inspect and explicitly resume deferred notifications; automatic receipt recovery cannot resume a deferred event. Each terminal notification records `wake_id`, `prompt_sha256`, and timing fields for worker observation, terminal assessment, pane validation, sidecar decisions, paste/submit, delivery capture, follow-up, receipt recheck, and total notification time so slow model calls can be separated from tmux delivery and Codex ack latency.
- Manager records must keep event lifecycle fields consistent across `events`, `notifications`, `last_notification`, and `last_ack`. `submitted_event_ids` records successful app-server delivery or tmux prompt submission attempts; `notified_event_ids` records events that reached a notification backend; neither field is Codex receipt. `acknowledged_by_codex` is receipt and must be reflected for the same event id in both notification and event summaries after `manager ack`.
...
- A single manager record owns multiple `job_id` entries in the current workspace/window. `manager submit --new-worker` starts parallel work in another visible worker pane; `manager submit --pane PANE_ID` binds work to a specific pane; compatibility `manager run-next` starts follow-up work in the default worker pane only when no manager job is active. `manager run-next` never creates a new worker pane, marks the previous terminal event as handled by the next job, and is blocked by any unacknowledged terminal event in bridge or tmux-inject mode. `manager cancel` stops only the manager by default; `--job-id` stops one worker job and `--all-workers` or `--stop-worker` attempts to stop worker jobs. Once cancellation is requested, that manager record keeps the cancel state even if delayed bridge acknowledgements or notification retries arrive.
- Completed jobs are preserved by default. In the manager viewer, `d`, `D`, Delete, or explicit cleanup commands remove only selected terminal job rows and their manager-owned command/status/log evidence. They never close panes/windows, stop the manager, or touch active jobs.
